...
const FUNCTION_SEARCH = /^(?:\d+)?(?:\s+)?(?:def\s+)(fn[\w](?:[\w\d]+)?(\$)?)(?:(?:\()([\w\d\$;&, *]+)(?:\)))?/img
  ...
  */
  ...
  getCurrentScopeVariables(request, completions){
    ...
    request.editor.backwardsScanInBufferRange(/^ *def *fn[a-z0-9_]+\$?(?:\*\d+)?\((?<params>[a-z$_,* &;\[\]]+)\)|\bfnend\b/ig,[[0,0],request.bufferPosition],({match,matchText,range,stop,replace}) => {
      if (matchText!=="fnend"){
        params = this.parseParams(match.groups.params)
        if (params){
  ...
  parseParams(paramString){
    ...
    let
      ...
      requiredParamsList = requiredParams ? requiredParams.split(",") : [],
      optionalParamsList = []

    // if optional params exist truncate anything after the '___'
    if (optionalParams){
      if (optionalParams.substring(0,3)==="___"){
        optionalParams = ""
      } else if (optionalParams.includes(",___")){
        [optionalParams] = optionalParams.match(/^.*(?=,___)/)
      }
      if (optionalParams){
        optionalParamsList = optionalParams.split(",")
      }
    }

    if (requiredParamsList){
      paramList.push(...requiredParamsList)
    }

    if (optionalParamsList){
      paramList.push(...optionalParamsList)
    }
