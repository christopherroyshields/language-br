...
const LIBRARY_REGEX = /^\s*(?:\d{1,5}\s+)?def\s+library\s+(?<name>fn\w+(?<isString>\$?))(?:\((?<required>[^;)]*)(?:;(?<optionalTruncated>[&\w,$* ]+),___|;___|;(?<optional>[&\w,$* ]+)\))?)?/img
...
module.exports = {
  ...
  */
      ...
      request.editor.scan(/library/,{
        ...
        for (var i = 0; i < functionList.length; i++) {
          ...
          */
          ...
          let func=lib.functions[functionList[i]]
          if (func){
  ...
  checkAndUpdateLibraries(request) {
    ...
    request.editor.scan(search,{},(match)=>{
              ...
              fs.readFile(sourcePath,(err,contents)=>{
                ...
                while (def = LIBRARY_REGEX.exec(source)) {
                  if (def.groups.required){
                    fn.requiredParams = def.groups.required.split(',')
                  }
                  if (def.groups.optionalTruncated){
                    fn.optionalParams = def.groups.optionalTruncated.split(",")
                  }
                  if (def.groups.optional){
                    fn.optionalParams = def.groups.optional.split(",")
                  }
                  this.loadedLibs[libPath].functions.push(fn)
                }
              })
            }
          });
