...
const CONTAINS_UNBALANCED_FN = /[a-zA-Z][a-zA-Z0-9_]*?\$?\([ ,0-9a-z]*\)/g
...
module.exports = {
  ...
  functionTooltip({editor, bufferPosition, prefix}){
    ...
    var paramCount = line.substring(fnStart).match(/,/g).length

    // look for unbalanced function call
    var enclosingFunctionList = line.match(/[a-zA-Z][a-zA-Z0-9_]*?\$?(?:\()/)
    console.log(enclosingFunctionList)

    if (enclosingFunctionList === null){
      return
    }

    // get nearest enclosing function
    var functionName = enclosingFunctionList[enclosingFunctionList.length-1]
    console.log(functionName)

    functionName = functionName.substring(0,functionName.length-1)
    console.log(functionName)

    // look up function in function list


    // create function  text

    // show tooltip
    this.functionHelperElement.innerText = functionName
    var editorElement = atom.packages.getLoadedPackage("autocomplete-plus").mainModule.autocompleteManager.suggestionList.suggestionListElement.element
    editor.getElement().classList.add('autocomplete-active')
    // suggestionList.items = [];
    // atom.packages.getLoadedPackage("autocomplete-plus").mainModule.autocompleteManager.suggestionList.show.call(suggestionList, editor, {prefix: "", items: })
    // const div1 = editor.decorateMarker(editor.getLastCursor().getMarker(), {
    //   type: "overlay",
    //   item: wrapper
    // })

    // debugger
    // editor.getElementsByClassName('autocomplete-plus');
    // var tip = atom.tooltips.add(div1, {title: functionName, trigger: "manual"})

    // create listener to destroy tooltip
  },
  getFunctionList(request){
    var fnlist = []
    fnlist = fnlist.concat(this.getInternal())
    return fnlist
  },
  ...
  getSuggestions (request, completions) {

    // this.functionHelperElement.innerHTML = ""
    // var fnCall = this.getFnCall(request)
    // console.log(fnCall);
    // if (fnCall.name || request.prefix.length >= 2){
    //   var functionList = this.getFunctionList(request)
    //   for (var i = 0; i < functionList.length; i++) {
    //     if (functionList[i].text.toLowerCase() === fnCall.name.toLowerCase()){
    //       if (functionList[i].params && functionList[i].params.length){
    //         this.functionHelperElement.insertAdjacentText('beforeend', `${ functionList[i].text }(`)
    //         for (var f = 0; f < functionList[i].params.length; f++) {
    //           if (f){
    //             this.functionHelperElement.insertAdjacentText('beforeend', ",")
    //           }
    //           if (f == fnCall.param){
    //             var paramEl = document.createElement("span")
    //             paramEl.innerText = functionList[i].params[f]
    //             paramEl.classList.add("current-param")
    //             this.functionHelperElement.insertAdjacentElement("beforeEnd", paramEl)
    //           } else {
    //             this.functionHelperElement.insertAdjacentText('beforeend', functionList[i].params[f])
    //           }
    //         }
    //         this.functionHelperElement.insertAdjacentText('beforeend', ")")
    //
    //         request.editor.decorateMarker(request.editor.getLastCursor().getMarker(), {
    //           type: "overlay",
    //           item: this.functionHelperElement
    //         })
    //         // for (var i = 0; i < array.length; i++) {
    //         //   if (fnCall.param == i){
    //         //     this.functionHelperElement.innerText.concat("(")
    //         //   } else {
    //         //     this.functionHelperElement.innerText.concat("(")
    //         //   }
    //         //   debugger
      ...
      return new Promise((resolve)=>{
        ...
        if (!this.inLibraryStatement(request)) {
          ...
          if (atom.config.get('language-br.includeInternalAutocomplete')) {
            completions = completions.concat(this.getInternal())   // Get the Internal Functions and Statements
          }
        ...
        if (atom.config.get('language-br.includeLibraryAutocomplete')) {
          ...
          // debugger
              resolve(completions)
              // this.functionTooltip(request, completions)

            })
        } else {
          resolve(completions)
          // this.functionTooltip(completions)
        }

      })
    } else {
      // this.functionTooltip(request, [])
    }
  },
  ...
  init(){
    ...
    // this.functionHelperElement = document.createElement("div")
    // this.functionHelperElement.classList.add("br-parameter-hint")
    // this.suggestionListElement = atom.packages.getLoadedPackage("autocomplete-plus").mainModule.autocompleteManager.suggestionList.suggestionListElement;
    //
    // atom.packages.getLoadedPackage("autocomplete-plus").mainModule.autocompleteManager.suggestionList.suggestionListElement.element.insertAdjacentElement("afterbegin", this.functionHelperElement)
