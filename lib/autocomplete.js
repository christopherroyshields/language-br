...
const PARAMS_SPLIT = /;/
const LIBRARY_REGEX = /def\s+library\s+(?<name>fn[\w]+)(?:\((?<params>.*)\))?/img
// const LIBRARY_REGEX = /^\s*(?:\d{1,5}\s+)?def\s+library\s+(?<name>fn\w+(?<isString>\$?))(?:\*\d+)?\s*\(?(?<required>[&\w$*, ]+)?;?(?<optional>[&\w$*, ]+)?\)?/img
  ...
  */
  ...
  getLayouts(request) {
    ...
    */
    ...
    if (this.infnOpen(request)) {
      ...
      for (var i = 0; i < this.layoutList[projectPath].length; i++) {
        // console.log(items[i])
        completions.push({
          text: this.layoutList[projectPath][i],
          type: 'constant',
          leftLabel: 'Layout'
        })
      }
      ...
      completions.concat(this.readCurrentLayoutsDetails(request,path.join(projectPath,layoutPath)))
    }
    return completions
  },
  ...
  getLibraryCompletions(request) {
    ...
    if (this.inLibraryStatement(request)) {
      ...
      var libPath = calculatePath(request,this.readLibraryStatementPath(request))
      var lib = this.loadedLibs[libPath]

      for (var i = 0; i < lib.functions.length; i++) {
        var functionDescription = this.calculateDescription(lib.functions[i])

        var completion = {
          text: lib.functions[i].name,
          type: 'function',
          snippet: lib.functions[i].name,
          //leftLabel: type,
          //rightLabel: null,
      ...
      var search = /(?:^|:)\s*\d{0,5}?\s*library\s+["'`](?<path>[^"'`]+)["'`]:(?<functionList>.*)$/img
      request.editor.scan(search,{
      },(match)=>{
        // for each thing found,
        // look up the function list and path to find the objects and return a detailed completion for each.
        var libPath = this.calculatePath(match.match.groups.path)
        var lib = this.loadedLibs[libPath]
        var functionList = match.match.groups.functionList.split(',')
        for (var i = 0; i < functionList.length; i++) {
          /*

          lib.functions[functionList[i]].name
          lib.functions[functionList[i]].requiredParams
          lib.functions[functionList[i]].optionalParams

          */

          var func=lib.functions[functionList[i]]
          var functionDescription = this.calculateDescription(func)
  ...
  checkAndUpdateLibraries(request) {
    ...
    // get root directory for current file
    //[projectPath] = atom.project.relativizePath(request.editor.getPath())
    ...
    var search = /(?:^|:)\s*\d{0,5}?\s*library\s+["'`](?<path>[^"'`]+)["'`]/img
    request.editor.scan(search,{
    },(match)=>{

      // determine library's full path by combinig project root library path
      var libPath = this.calculatePath(match.match.groups.path.replace("\\","/"))
          ...
          fs.readFile(sourcePath,(err,contents)=>{
            if (err){
              ...
              while (def = LIBRARY_REGEX.exec(source)) {
                let requiredParams = []
                let optionalParams = []
                if (def.groups.params){
                  if (def.groups.params.includes(';')){
                    let splitParams = def.groups.params.split(';')
                    let optionalParams = splitParams[1].split(',')

                    if (optionalParams.indexOf("___")){
                      optionalParams.splice(optionalParams.indexOf("___"),optionalParams.length-1)
                    }

                    if (splitParams[0]){
                      requiredParams = splitParams[0].split(',')
                    }
                  } else {
                    requiredParams = def.groups.params.split(',')
                  }
                  this.loadedLibs[libPath].functions.push({
  ...
  getLibraryFunctions(request) {
    ...
    // if (this.checkAndUpdateLibraries(request)) {
    //
    // }
    /*
