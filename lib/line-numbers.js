...
function getLines(bufferText) {
    ...
    while ((match = NOT_IN_STRING.exec(bufferText)) !== null) {
      ...
      // keep track of any multiLineStringCount
      if (/(^`.*\n.*`$|^`.*\n.*{{$|}}.*\n.*{{|^}}.*\n.*`$)/s.test(match[0])){
        multiLineStringCount += match[0].split(/\r?\n/).length - 1
      }
      ...
      for (var i = 1; i < lines.length; i++) {
        if (LINE_IS_CONTINUATED.test(code[i-1])){
          ...
          const MULTILINE_COMMENT_END = /(.*)\*\/(.*)/s
          if (inMultiLineComment){
            if (MULTILINE_COMMENT_END.test(newLine)){
              inMultiLineComment = false
              newLine = newLine.replace(MULTILINE_COMMENT_END, "$2 ! $1")
            } else {
              newLine = " ! " + newLine
            }
          } else {
            if (MULTILINE_COMMENT.test(newLine)){
              inMultiLineComment = true
              newLine = newLine.replace(MULTILINE_COMMENT, "!")
            }
          }

          newLines[lineCount.sourceLine] = [lineCount.count, newLine]

          lineCount.count += lineCount.increment

          if (multiLineStringCount && i === 1){
            lineCount.sourceLine += multiLineStringCount
            multiLineStringCount = 0
          }

          lineStack = code[i]
          commentStack = comment[i]
          lineCount.sourceLine += 1 + contCount
          contCount = 0
        }
      }
