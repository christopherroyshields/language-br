...
const HAS_AUTONUMBER_IN_COMMENT = /(!|^[ \t]*rem).*#autonumber#[ \t]*(?<lineCount>\d+),[ \t]*(?<increment>\d+)/im
...
function codeReplace(bufferText, fn){
  let
    match,
    chunkStart = 0,
    chunkIndex = 0
  ...
  while ((match = NOT_IN_STRING.exec(bufferText)) !== null) {
    ...
    let newChunk = fn(chunk, chunkIndex)

    bufferText = replaceBetween(bufferText, chunkStart, match.index, newChunk)

    NOT_IN_STRING.lastIndex += newChunk.length - chunk.length
...
function addLineForChunk(chunk, chunkIndex){

  let newChunk = chunk.replace(LINES_NEEDING_NUMBERS, function(lineMatch, p1, p2, index, source){

    // check for autonumber
    let autoNumberMatch = HAS_AUTONUMBER_IN_COMMENT.exec(lineMatch)
    if (autoNumberMatch){
      if (parseInt(autoNumberMatch.groups.lineCount) > lineCount){
        lineCount = parseInt(autoNumberMatch.groups.lineCount)
        increment = parseInt(autoNumberMatch.groups.increment)
      }
    }

    // comment to blank line
    let addedCommentChar = STRING_EMPTY
    // if (p2.trim() === STRING_EMPTY){
    //   addedCommentChar = "!"
    // }

    let withLinesChunk = `${p1}${lineCount.toString().padStart(5,"0")} ${addedCommentChar}${p2}`
    lineCount += increment
    return withLinesChunk
  })
...
module.exports = {
  addLineNumbers(editor) {
    try {
      ...
      let newBufferText = codeReplace(bufferText, addLineForChunk)

      newBufferText = newBufferText.substring(1)
      if (addLine){
        newBufferText = newBufferText + os.EOL
      }
