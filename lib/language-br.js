...
module.exports = {
  ...
  lookup(cursor, step) {
    ...
    this.animateSelection('occurence-current');

    debugger
    return results;
  },

  animateSelection(animationClassName) {
    var range = this.editor.getSelectedBufferRange()
    if (this.marker){
    ...
    if (this.decoration){
      this.decoration.setProperties({type: 'text', class: animationClassName})
    } else {
      this.decoration = this.editor.decorateMarker(this.marker, {type: 'text', class: animationClassName})
    }
    // setTimeout(function () {
