...
module.exports = {
  ...
  cloudCompile(editor, sourcePath){
    ...
    fetch("http://127.0.0.1:3000/compile", requestOptions)
      ...
      .then(result => {
        ...
        if (error && !bin.length){
          atom.notifications.addError(`Compile Error!`,{
            dismissable: true,
            detail: `Error ${error} occured on line ${sourceLine}.`
          })
        ...
        if (error && bin.length){
          var message = `Compile warning:\n Error ${error}`
          atom.notifications.addWarning('Compile Warning', {
            dismissable: true,
            detail: `Error ${error} occured.`
          })
        }
      ...
      if (prog && lexiCloudEnabled){
        if (path.extname(prog)===".br" || path.extname(prog)===".wb"){
          ...
          atom.workspace.open().then((editor)=>{
            ...
            fetch(`http://${lexiCloud}:3000/decompile`, requestOptions)
              .then(response => response.text())
              .then(result => {
