...
module.exports = {
  ...
  cloudCompile(editor, sourcePath){
    ...
    fetch(`https://${lexiUrl}/compile`, requestOptions)
      ...
      .then(result => {
        ...
        if (bin && !error){
          ...
          editor.errmarker.setScreenRange([
            [0,0]
            [0,0]
          ])
        }
        ...
        if (error && !bin){
          ...
          if (sourceLine){
            editor.setCursorScreenPosition([sourceLine-1,1])
            editor.moveToEndOfLine()
            var lineText = editor.lineTextForBufferRow(sourceLine-1)
            editor.errmarker.setScreenRange([
              [
                sourceLine-1,
                lineText.search(/[^ ]/)
    ...
    atom.workspace.observeTextEditors((editor)=>{
      if (editor){
        ...
        if (path.extname(prog)===".brs" || path.extname(prog)===".wbs"){
          ...
          editor.errmarker = editor.markScreenRange([[0,0],[0,0]], {
      ...
      if (prog && lexiCloudEnabled){
        if (path.extname(prog)===".br" || path.extname(prog)===".wb"){
          ...
          atom.workspace.open().then((editor)=>{
            ...
            fetch(`https://${lexiCloud}/decompile`, requestOptions)
              .then(response => response.text())
              .then(result => {
