...
function addGutter(editor){
  ...
  editor.onDidChange(() => {
    lines = LineNumbers.getLines(editor.getText())
    updateGutter(editor)
  })

  editor.onDidChangeSelectionRange((e)=>{
    updateGutter(editor)
  })

  let gutter = editor.addGutter({
    name: "br-line-numbers",
    type: "line-number",
    priority: 1,
    visible: true,
    class: "br-line-number-gutter",
    onMouseDown: ({bufferRow, screenRow, domEvent}) => {
      gutter.gutterContainer.textEditor.component.didMouseDownOnLineNumberGutter(domEvent)
    },
    labelFn: ({bufferRow, screenRow, foldable, softWrapped, maxDigits}) => {
      if (lines[bufferRow]){
        return lines[bufferRow][0].toString().padStart(5,"0")
      } else {
        return " "
      }
    }
...
function updateGutter(editor){
  let gutter = editor.gutterWithName("br-line-numbers")

  if (editor.brLineGutterDecorations){
    editor.brLineGutterDecorations.forEach((decoration, i) => {
      decoration.destroy()
    });
  }

  editor.brLineGutterDecorations = []

  let foldedLayerMarkers = editor.displayLayer.foldsMarkerLayer.getMarkers()
  foldedLayerMarkers.forEach((folded, i) => {
    editor.brLineGutterDecorations.push(
      gutter.decorateMarker(folded, {
        type: 'gutter',
        class: 'folded'
      })
    )
  ...
  let selectionMarkers = editor.selectionsMarkerLayer.getMarkers()

  selectionMarkers.forEach((marker, i) => {
    editor.brLineGutterDecorations.push(
  ...
  // spoof decoration change to trigger gutter update
  if (!editor.spoofMarker){
    editor.spoofMarker = editor.markBufferRange([[0, 0], [0, 0]])
  }

  if (editor.spoofDecoration){
    editor.spoofDecoration.destroy()
  }

  if (!editor.spoofCount){
    editor.spoofCount = 0
  }

  let cls = "spoof-" + (editor.spoofCount++).toString()

  editor.spoofDecoration = gutter.decorateMarker(editor.spoofMarker, {
    type: "gutter",
    class: cls
  })
}
...
module.exports = {
  ...
  config: {
    "brLineNumbers": {
      "description": "Show projected BR line numbers",
      "type": "boolean",
      "default": false
    },
  ...
  activate(state) {
    ...
    atom.workspace.observeTextEditors((editor)=>{
      editor.brLineGutter = addGutter(editor)
      // if (isBrSource(editor) && atom.config.get('language-br.brLineNumbers')){
      //   editor.brLineGutter.show()
      //   editor.gutterWithName("line-numbers").hide()
      //   editor.updateGutter(editor)
      // }
    })

    atom.config.observe('language-br.brLineNumbers', (newValue) => {
      console.log(`gutter ${newValue}`);
      atom.workspace.getTextEditors().forEach((editor, i) => {
        if (newValue){
          editor.brLineGutter.show()
        } else {
          editor.brLineGutter.hide()
        }
      });
    ...
    atom.commands.add('atom-workspace', {
      'language-br:toggle-line-number-gutter': function() {
        let currentSetting = atom.config.get("language-br.brLineNumbers")
        atom.config.set("language-br.brLineNumbers", !currentSetting)
        // let
        //   editor = atom.workspace.getActiveTextEditor(),
        //   source = editor.getText(),
        //   gutter = editor.gutterWithName("br-line-numbers")
        //
        // if (!gutter){
        //   updateGutter(editor)
        // }
